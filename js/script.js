...
function setLanguage(lang) {
  ...
  console.log("local storage :", localStorage.getItem("selectedLang"));
  applyTranslations(lang);
  updateLanguageStyles(lang);
}
const translations = {
...
function applyTranslations(lang) {
  ...
  const telpElement = document.getElementById("telp");
  const telpNumber = translations[lang].telp.replace(/[\s-]+/g, ""); // Remove spaces
  if (telpElement) telpElement.href = `https://wa.me/${telpNumber}`;
}

// Send Via WhatsApp (Order Product Button)
function getPhoneNumber() {
  let lang = "id";
  if (translations[lang] && translations[lang].telp) {
    return translations[lang].telp.replace(/[\s-]+/g, ""); // Remove spaces and hyphens
  } else {
    console.error("Phone number not found for the selected language.");
    return null;
  }
}
...
function sendToWhatsApp() {
  ...
  const telpNumber = translations["id"].telp.replace(/[\s-]+/g, ""); // Remove spaces

  const whatsappMessage = `Halo, Nama saya ${fullname}\n\n${message}`;

  const whatsappURL = `https://wa.me/${telpNumber}?text=${encodeURIComponent(
    whatsappMessage
  )}`;
